...
double PayoffFunctions::revisit_time_norm(double rev_time)
{
    if(rev_time < 0.f) {
        return 1.f;
    }
...
void PayoffFunctions::bindPayoffFunctions(void)
{
    ...
    f_revisit_time_backwards.first = [](PFArg0 ats, PFArg1 bts, PFArg2 bs) {
        ...
        for(unsigned int i = 0; i < bs.size(); i++) {
            ...
            for(unsigned int j = 0; j < bts[i].size(); j++) {
                if(b_act_ptr->isFact() && b_act_ptr->isConfimed()) {
                    double t_diff_i = ats.first - bts[i][j].second;
         ...
         *      - Start (t0) after `a`.
         **/
        double rvtb = revisit_time_norm(t_diff); /* Revisit time backwards. This is the worst case (normalized). */
